import {
  AfterViewInit,
  Component, ContentChild,ɵNoopNgZone,
  ElementRef,
...
export class AutocompleteComponent implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor {
  ...
  @ViewChild('filteredListElement')
  filteredListElement!: ElementRef; // element of items
  @ViewChild('historyListElement')
  ...
  private manualOpen = false;  // = undefined;
  ...
  @Input()
  public searchKeyword!: string; // keyword to filter the list
  @Input() public placeHolder = ''; // input placeholder
  @Input() public heading = '';
  @Input()
  public initialValue!: never; // set initial value
   ...
   */
  @Input()
  public historyIdentifier!: string;
  /**
  ...
  @Input()
  public isLoading!: boolean; // loading mask
  @Input()
  public debounceTime!: 400; // delay time while typing
  @Input()
  public disabled!: boolean; // input disable/enable
  /**
  ...
  public select(item: never  ) {
    ...
    if (this.initialValue) {
      ...
      if (history) {
        ...
        if (!existingHistory.some((existingItem: { [x: string]: any; }) => !this.isType(existingItem)
   ...
   * Define panel state
   */
  setPanelState(event: Event | undefined) {
    if (event) {
